...
@DataJpaTest
class StudentSeesTeacherAnswerServiceSpockPerformanceTest extends Specification {
    ...
    def "performance testing COUNT teachers answering discussions"() {
        given: "COUNT students, 1 teachers and COUNT questions"
        createUser(COUNT+1, User.Role.TEACHER)
        1.upto(COUNT, {
            createUser(it, User.Role.STUDENT)
            createQuestion(it)
        })
        and: "COUNT discussions with teacher answers"
        1.upto(COUNT, {
            createDiscussion(it)
        })
        when:
        1.upto(COUNT, {
            discussionService.getDiscussionStudent(students[it].getId())
        })
        then:
        true
    }
    ...
    def createUser(it, role) {
        User user = new User(USER_NAME, USER_NAME + it, it, role)
