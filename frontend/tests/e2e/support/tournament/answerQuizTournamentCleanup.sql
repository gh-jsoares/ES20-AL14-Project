DELETE FROM question_answers WHERE quiz_answer_id IN (SELECT id FROM quiz_answers WHERE quiz_id IN (SELECT quiz_id FROM tournaments WHERE title='T1-Cypress Test'));
