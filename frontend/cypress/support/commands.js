...
Cypress.Commands.add('createNewTournament', (title, topics, questions, scramble, start, end) => {
    if (!!title) cy.get('[data-cy="Title"]').type(title)
    for (let t of topics)
        cy.get('[data-cy="Topics"]').type(t+'{enter}'+'{backspace}'.repeat(t.length))
    cy.get('[data-cy="Topics"]').type('{esc}')
    if (!!questions) {
        cy.get('[data-cy="QuestText"]').type('{backspace}' + questions)
    }
    if (!!scramble) cy.get('[data-cy="Scramble"]').parent().click()

    if (!!start) {
        cy.contains('*Start Date').parent().click()
        cy.selectTournamentDate(start);
    }
    if (!!end) {
        cy.contains('*Conclusion Date').parent().click()
...
Cypress.Commands.add('selectTournamentDate', (date) => {
    cy.get('.v-dialog--active').within(() => {
        if (date === 0)
            ...
            cy.wait(500)
            cy.get('.v-date-picker-table').contains('1').click();
        }
